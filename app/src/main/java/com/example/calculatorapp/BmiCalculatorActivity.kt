...
class BmiCalculatorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        heightSelect.setOnClickListener{
            val bottomSheet=BottomSheetDialog(this@BmiCalculatorActivity,R.style.BottomSheetDialogTheme)
            val bottomSheetView=LayoutInflater.from(applicationContext)
                .inflate(R.layout.bottom_sheet_height,findViewById(R.id.bottomSheet))as ConstraintLayout
        ...
        linearLayout2.setOnClickListener {
            if(!man && !girl){
            ...
            else if(heigh_value.text.toString() == "00"){
                Toast.makeText(this,"Height is Incorrect", Toast.LENGTH_SHORT).show();
            }else if(weight_value.text.toString() == "00"){
                ...
                if(statusBmi=="Thin" ||statusBmi=="Normal"||statusBmi=="Fat"){
                    ...
                    val bottomSheet=BottomSheetDialog(this@BmiCalculatorActivity,R.style.BottomSheetDialogTheme)
                    val bottomSheetView=LayoutInflater.from(applicationContext)
                        .inflate(R.layout.bottom_sheet_bmi,findViewById(R.id.bottomSheetBmi))as ConstraintLayout
                    val resultBmiTextView =bottomSheetView.findViewById<TextView>(R.id.resulBmi)
                    resultBmiTextView.text=bmivalue.toString()
                    val statusBmiText=bottomSheetView.findViewById<TextView>(R.id.status)
                    statusBmiText.text=statusBmi

                    if(statusBmi=="Thin"){
                        resultBmiTextView.setTextColor(resources.getColor(R.color.light_blue))
                    }else if(statusBmi=="Normal"){
                        resultBmiTextView.setTextColor(resources.getColor(R.color.green))

                    }else{
                        resultBmiTextView.setTextColor(resources.getColor(R.color.red))
                    }
                    val genderBmiText=bottomSheetView.findViewById<TextView>(R.id.gender)
                    if(man){
                        genderBmiText.setTextColor(resources.getColor(R.color.blue))
                        genderBmiText.text="Male"

                    }else{
                        genderBmiText.setTextColor(resources.getColor(R.color.pink))
                        genderBmiText.text="Female"
                    }
                    bottomSheetView.findViewById<View>(R.id.linearLayout2).setOnClickListener {
                        bottomSheet.dismiss()
                    }
